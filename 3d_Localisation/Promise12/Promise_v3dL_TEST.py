...
if __name__ == '__main__':
    ...
    Arg_parser.add_argument('-d','--data_path',help='Path to data (location of data)',default = '/path/to/the/DATA/Promise12/test')
    ...
    train = False
    data_sz = [64, 256, 256]
    csv_META_file = 'promise12_test.csv'
    ...
    weight_path = os.path.join("weights",args.codeName+'.pt')
    print(csv_META_file,'\n',weight_path)
    ...
    Promise_3D_model.load_state_dict(torch.load(weight_path))
    print('model loaded!!!!!........................... ')
    
    avJ = 0
    ctoc_dist = np.zeros((len(df)))
    #...............
    ...
    for VAL_vol, VAL_GT_st, VAL_GT_end, VAL_GT_Xst, VAL_GT_Xend, VAL_GT_Yst, VAL_GT_Yend in test_dLoader:
        ...
        with torch.no_grad():
            ...
            VAL_pred_st, VAL_pred_end, VAL_pred_Xst, VAL_pred_Xend, VAL_pred_Yst, VAL_pred_Yend = Promise_3D_model(VAL_vol)
    ...
    df_save.to_csv(os.path.join("weights","Result",args.codeName+'.csv'), encoding='utf-8', index=False)
