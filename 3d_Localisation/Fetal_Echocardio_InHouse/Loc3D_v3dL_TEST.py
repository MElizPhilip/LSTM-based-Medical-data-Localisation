...
if __name__ == '__main__':
    ...
    for fold in range(1,args.NFold+1):
        csv_META_file = 'folds_csv/loc_F'+str(fold)+'_test.csv'
        ...
        cnn_lstm_model = inHouse_3D_LOC(args.data_size,args.kSz)
        
        if torch.cuda.is_available():
            cnn_lstm_model.cuda()
        cnn_lstm_model.load_state_dict(torch.load(weight_path))
        print('model loaded!!!!!........................... ')
        ...
        for VAL_vol, VAL_GT_st, VAL_GT_end, VAL_GT_Xst, VAL_GT_Xend, VAL_GT_Yst, VAL_GT_Yend in test_dLoader:
            ...
            with torch.no_grad():
                ...
                VAL_pred_st, VAL_pred_end, VAL_pred_Xst, VAL_pred_Xend, VAL_pred_Yst, VAL_pred_Yend = cnn_lstm_model(VAL_vol)
            pred_st = int((VAL_pred_st.item()+corr) *data_size)
            pred_end = int((VAL_pred_end.item()+corr)*data_size)
            pred_Xst = int((VAL_pred_Xst.item()+corr) *data_size)
            pred_Xend = int((VAL_pred_Xend.item()+corr)*data_size)
            pred_Yst = int((VAL_pred_Yst.item()+corr) *data_size)
            pred_Yend = int((VAL_pred_Yend.item()+corr)*data_size)
            ...
            gt_Xst = int((VAL_GT_Xst.item()+corr)*data_size)
            gt_Xend = int((VAL_GT_Xend.item()+corr)*data_size)
            gt_Yst = int((VAL_GT_Yst.item()+corr)*data_size)
            gt_Yend = int((VAL_GT_Yend.item()+corr)*data_size)
            ...
            ctoc_dist[cnt_vol] = np.linalg.norm(Cent_gt - Cent_pred) * 0.5 # 0.5 is voxel spacing
            cnt_vol += 1
